...
def grafico_dispersao(df):
    ...
    """
    if "Sleep_Hours_per_Night" not in df.columns or "Final_Score" not in df.columns:
        print("Erro: As colunas necessárias para o gráfico de dispersão não foram encontradas no dataset.")
        return
    
    if df["Sleep_Hours_per_Night"].empty or df["Final_Score"].empty:
        print("Erro: As colunas para o gráfico de dispersão estão vazias.")
        return
...
def grafico_barras_idade_x_midterm(df):
    ...
    Args:
        df (pd.DataFrame): DataFrame contendo os dados limpos.
    """
    if "Age" not in df.columns or "Midterm_Score" not in df.columns:
        print("Erro: As colunas necessárias para o gráfico de barras não foram encontradas no dataset.")
        return
    
    if df["Age"].empty or df["Midterm_Score"].empty:
        print("Erro: As colunas para o gráfico de barras estão vazias.")
        return
...
def grafico_pizza_idades(df):
    ...
    if "Age" not in df.columns:
        print("Erro: A coluna 'Age' necessária para o gráfico de pizza não foi encontrada no dataset.")
        return
